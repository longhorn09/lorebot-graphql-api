DELIMITER $$
CREATE DEFINER=`ntang`@`%` PROCEDURE `CreatePerson_v002`(
    charName varchar(30),	
    light varchar(100),
    ring1 varchar(100),
    ring2 varchar(100),
    ...
    if (isExists > 0) then
		begin
            ...
            SET Person.LIGHT=light,
                ...
                Person.ON_CHEST = onchest
            WHERE Person.PERSON_ID = updateID
            LIMIT 1;

        end;
	else
		INSERT INTO Person(CHARNAME,LIGHT,RING1,RING2,NECK1,NECK2,BODY,HEAD,LEGS,FEET,ARMS,SLUNG,HANDS,SHIELD,ABOUT,WAIST,POUCH,RWRIST,LWRIST,PRIMARY_WEAP,SECONDARY_WEAP,HELD,BOTH_HANDS,SUBMITTER,CLAN_ID,ON_CHEST)
        VALUES(charName, light, ring1, ring2, neck1, neck2, body, head, legs, feet, arms, slung, hands, shield, about, waist, pouch, rwrist, lwrist, weap1, weap2, held, both_hands, submitter, clan_id, onchest);
    end if;
