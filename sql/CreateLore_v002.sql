DELIMITER $$
CREATE DEFINER=`ntang`@`%` PROCEDURE `CreateLore_v002`(
	objName varchar(100),
    itemType varchar(30),
    itemIs varchar(80),
    ...
    if (isExists > 0) then
		begin
            ...
            SET Lore.ITEM_TYPE=itemType,
                ...
                Lore.POWER = power,
                Lore.DAMAGE = damage,
                Lore.CAN_USE = can_use
            WHERE Lore.LORE_ID = updateID
            LIMIT 1;
		end;
	else
		INSERT INTO Lore(OBJECT_NAME,ITEM_TYPE,ITEM_IS,SUBMITTER,AFFECTS,APPLY,RESTRICTS,CREATE_DATE,CLASS,MAT_CLASS,MATERIAL,ITEM_VALUE,EXTRA,IMMUNE,EFFECTS,WEIGHT,CAPACITY,ITEM_LEVEL,CONTAINER_SIZE,CHARGES,SPEED,ACCURACY,POWER,DAMAGE,CAN_USE)
        VALUES(              objName,itemType ,itemIs ,submitter,affects,apply,restricts,NOW()  ,weapClass,matClass ,material, itemValue,extra,immune,effects,weight,capacity,itemLevel,containerSize,charges,speed,accuracy,power,damage,can_use);
	end if;
END$$
DELIMITER ;
